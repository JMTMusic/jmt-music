...
export function CreateProjectForm() {
  ...
  return (
    <form action={action} className="mt-8 grid gap-5">
      <label className="text-sm font-medium text-slate-300">Artist/client name<input className={fieldClass} name="artistName" required minLength={2} maxLength={120} autoComplete="organization" /></label>
      ...
      <button disabled={pending} className="mt-2 rounded-xl bg-sky-300 px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-sky-200 disabled:cursor-wait disabled:opacity-60">{pending ? "Creating…" : "Create Project"}</button>
