...
export async function updatePortalStageAction(_previous: StageActionState, formData: FormData): Promise<StageActionState> {
  ...
  try {
    ...
    const { error } = await supabase.from("project_portal_stages").upsert({ project_id: projectId, property_id: property.id, stage, status, client_note: clientNote, updated_at: new Date().toISOString() });
