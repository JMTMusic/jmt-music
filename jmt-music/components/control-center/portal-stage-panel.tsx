...
function StageRow({ item, propertyId, projectId, canEdit }: { item: PortalStage; propertyId: SiteId; projectId: string; canEdit: boolean }) {
  ...
  return <form action={action} className="grid gap-3 rounded-xl border border-white/8 bg-white/[.025] p-4 md:grid-cols-[140px_180px_90px_1fr_auto] md:items-end">
    ...
    <label className="grid gap-1 text-xs text-slate-400">Progress %<input className={field} name="progressPct" type="number" min={0} max={100} defaultValue={item.progressPct} disabled={!canEdit} /></label>
    ...
    {state.message && <p className={`text-xs md:col-span-5 ${state.status === "error" ? "text-red-300" : "text-emerald-300"}`}>{state.message}</p>}
  </form>;
}
