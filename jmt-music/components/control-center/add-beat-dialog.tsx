...
/** Add Beat dialog with square artwork processing and direct signed Storage uploads. */
export function AddBeatDialog({ propertyId, disabled = false }: { propertyId: SiteId; disabled?: boolean }) {
  ...
  const submitBeat = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const created = await createBeat(initialState, formData);
      setState(created);
    ...
    <>
      ...
      {open && <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/75 p-4 backdrop-blur-sm" role="presentation">
        <section role="dialog" aria-modal="true" aria-labelledby="add-beat-title" className="my-8 w-full max-w-3xl rounded-2xl border border-white/10 bg-[#0a0f16] shadow-2xl">
          <header className="flex items-start justify-between border-b border-white/8 p-6"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-sky-300">Beat Library</p><h2 id="add-beat-title" className="mt-2 font-sans text-2xl font-semibold">Add Beat</h2><p className="mt-2 text-sm text-slate-500">Create the beat, square artwork, and optional audio preview.</p></div><button type="button" onClick={() => setOpen(false)} disabled={pending} aria-label="Close Add Beat dialog" className="rounded-lg border border-white/10 p-2 text-slate-400 hover:text-white"><X className="h-4 w-4" /></button></header>
          <form ref={formRef} onSubmit={submitBeat} className="p-6">
            ...
            <div className="mb-6 grid gap-4 rounded-xl border border-white/8 bg-white/[0.02] p-4 sm:grid-cols-[120px_1fr] sm:items-center">
              <div className="aspect-square overflow-hidden rounded-xl border border-white/10 bg-black/25">{artworkPreview ? <img src={artworkPreview} alt="Cropped cover artwork preview" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center px-3 text-center text-[10px] uppercase tracking-wider text-slate-600">Artwork preview</div>}</div>
              <label className={labelClass}>Cover artwork<input className={`${inputClass} cursor-pointer py-2 file:mr-3 file:rounded-lg file:border-0 file:bg-sky-300/10 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-sky-200`} name="artwork" type="file" accept="image/jpeg,image/png,image/webp,image/avif,.jpg,.jpeg,.png,.webp,.avif" onChange={selectArtwork} /><span className="mt-2 block text-[11px] font-normal text-slate-500">Square WebP output · 1200×1200 · Maximum 10 MB</span>{(artworkError || state.fieldErrors?.artwork) && <span className="mt-1 block text-[11px] text-red-300">{artworkError || state.fieldErrors?.artwork}</span>}</label>
            </div>
            <div className="mb-6 rounded-xl border border-white/8 bg-white/[0.02] p-4">
              <label className={labelClass}>Audio preview<input className={`${inputClass} cursor-pointer py-2 file:mr-3 file:rounded-lg file:border-0 file:bg-sky-300/10 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-sky-200`} name="audio" type="file" accept="audio/mpeg,audio/wav,audio/mp4,.mp3,.wav,.m4a" onChange={selectAudio} /><span className="mt-2 block text-[11px] font-normal text-slate-500">{audio ? `${audio.name} · ${(audio.size / 1024 / 1024).toFixed(1)} MB` : "MP3, WAV, or M4A · Maximum 100 MB"}</span>{audioError && <span className="mt-1 block text-[11px] text-red-300">{audioError}</span>}</label>
              {audioPreview && <audio className="mt-4 w-full" controls preload="metadata" src={audioPreview} />}
            </div>
