...
import { BarChart3, FileText, Globe2, LayoutDashboard, Menu, Music2, Search, Settings, Users, X } from "lucide-react";
...
const navigation: NavigationItem[] = [
  ...
  { label: "Clients", href: "/control-center/clients", icon: Users },
    ...
    <div className="min-h-screen bg-[#05080d] font-sans text-slate-100">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 border-r border-white/8 bg-[#070b11]/95 p-5 backdrop-blur-2xl transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-14 items-center justify-between">
          <Link href="/control-center" className="flex items-center gap-3" onClick={() => setOpen(false)}>
            <span className="grid h-10 w-10 place-items-center border border-sky-300/60 font-serif text-xs text-sky-300">JMT</span>
            <span><strong className="block text-sm tracking-[.12em]">CONTROL CENTER</strong><small className="text-[10px] text-slate-500">JMT Music</small></span>
          </Link>
        ...
        <nav className="mt-10 space-y-1" aria-label="Control Center">
          {navigation.map(({ label, href, icon: Icon }) => {
            ...
            return <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm transition ${active ? "bg-sky-300/12 text-sky-200 shadow-[inset_0_0_0_1px_rgba(125,211,252,.12)]" : "text-slate-400 hover:bg-white/[0.04] hover:text-white"}`}><Icon className="h-[18px] w-[18px]" />{label}</Link>;
          })}
        </nav>
        <div className="absolute inset-x-5 bottom-5 rounded-xl border border-white/8 bg-white/[0.025] p-4"><div className="flex items-center gap-2 text-xs font-medium text-emerald-300"><span className="h-2 w-2 rounded-full bg-emerald-400" />Production online</div><p className="mt-2 text-[11px] text-slate-500">Analytics services connected</p></div>
      </aside>
