      ...
      <div className="min-w-0">
        <section className={`${workspacePanel} overflow-hidden p-[16px_18px_18px]`}>
          {selected ? <>
            ...
            <audio key={selected.id} ref={audio} src={selected.previewAudioUrl || undefined} preload="metadata" onLoadedMetadata={(event) => setDuration(Number.isFinite(event.currentTarget.duration) ? event.currentTarget.duration : 0)} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => setPlaying(false)} />
