...
function PortalFileRow({ file, propertyId, projectId, canEdit }: { file: PortalFile; propertyId: SiteId; projectId: string; canEdit: boolean }) {
  ...
  if (editing) {
    return <form action={editAction} className="grid gap-2 rounded-xl border border-sky-300/25 bg-sky-300/[.04] p-3 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-xs text-slate-400">Title<input className={inputClass} name="title" defaultValue={file.title} required maxLength={160} /></label>
      <label className="grid gap-1 text-xs text-slate-400">Version<input className={inputClass} name="versionLabel" defaultValue={file.versionLabel || ""} /></label>
      <label className="grid gap-1 text-xs text-slate-400">BPM<input className={inputClass} name="bpm" defaultValue={file.bpm || ""} maxLength={20} /></label>
      <label className="grid gap-1 text-xs text-slate-400">Key<input className={inputClass} name="musicalKey" defaultValue={file.musicalKey || ""} maxLength={20} /></label>
      <label className="grid gap-1 text-xs text-slate-400 md:col-span-2">Google Drive link<input className={inputClass} name="driveUrl" type="url" defaultValue={file.driveUrl} required /></label>
      <div className="flex flex-wrap items-center gap-3 md:col-span-2">
        <button disabled={editPending} className="inline-flex min-h-9 items-center gap-2 rounded-lg bg-sky-300 px-3 text-xs font-semibold text-slate-950 disabled:opacity-60">{editPending && <LoaderCircle className="h-3.5 w-3.5 animate-spin" />}Save</button>
  ...
  return <div className="rounded-xl border border-white/8 bg-white/[.025] p-3 text-sm">
    <div className="flex items-center justify-between gap-4">
      <a href={file.driveUrl} target="_blank" rel="noopener noreferrer" className="flex min-w-0 flex-1 items-center gap-2 hover:text-sky-300">
      ...
      {canEdit && <div className="flex shrink-0 items-center gap-2">
        {file.fileType === "audio" && <>
          ...
          <button type="button" disabled={uploading} onClick={() => fileInputRef.current?.click()} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-white/15 px-2.5 text-xs text-slate-300 disabled:opacity-60">{uploading ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}Audio</button>
        </>}
        <button type="button" onClick={() => setEditing(true)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-white/15 px-2.5 text-xs text-slate-300"><Pencil className="h-3.5 w-3.5" />Edit</button>
      </div>}
    </div>
...
export function PortalFilePanel({ propertyId, projectId, files, canEdit, schemaUnavailable }: { propertyId: SiteId; projectId: string; files: PortalFile[]; canEdit: boolean; schemaUnavailable: boolean }) {
  ...
  return <div>
    ...
    {canEdit && <form action={action} className="mt-5 grid gap-3 border-t border-white/8 pt-5 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-xs text-slate-400">BPM<input className={inputClass} name="bpm" placeholder="120" maxLength={20} /></label>
      <label className="grid gap-1 text-xs text-slate-400">Key<input className={inputClass} name="musicalKey" placeholder="F minor" maxLength={20} /></label>
