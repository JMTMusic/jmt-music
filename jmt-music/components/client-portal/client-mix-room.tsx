...
export function ClientMixRoom({ view, accessToken, clientName, readOnly = false }: { view: ClientPortalView; accessToken: string; clientName: string; readOnly?: boolean }) {
  const { songs, files } = view;
  ...
  return <div className="pb-12">
    ...
    <div className="mt-[26px] grid gap-[18px] xl:grid-cols-[214px_minmax(0,1fr)_316px]">
      <aside className={`${workspacePanel} self-start overflow-hidden`}>
        ...
        </button>)}</div> : <div className="p-5 text-xs leading-5 text-slate-500">No songs have been shared yet.</div>}
      </aside>
      ...
      <div className="min-w-0">
        <section className={`${workspacePanel} overflow-hidden p-[16px_18px_18px]`}>
          {selected ? <>
            <div className="mb-4 flex flex-wrap items-start gap-3">
              <div className="min-w-[180px] flex-1"><h2 className="font-serif text-[21px] leading-tight text-white">{selected.title}</h2><p className="mt-1 text-[11.5px] text-slate-500">{[view.client.artistName, selected.bpm, selected.musicalKey, selected.versionLabel].filter(Boolean).join(" · ")}</p></div>
        ...
        {selected && <section className={`${workspacePanel} mt-[18px] overflow-hidden`}><PanelTitle>Files — {selected.title}</PanelTitle>{songFiles.length ? <div className="divide-y divide-white/8">{songFiles.map((file) => <a key={file.id} href={file.driveUrl} target="_blank" rel="noopener noreferrer" className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-white/[.035]"><span className="grid h-9 w-9 place-items-center rounded-[5px] bg-blue-400/10 text-blue-300"><FileAudio className="h-4 w-4" /></span><span className="min-w-0 flex-1"><strong className="block truncate text-xs text-slate-100">{file.title}</strong><span className="text-[10px] text-slate-500">{file.versionLabel || file.fileType} · Added {date(file.createdAt)}</span></span><Download className="h-4 w-4 text-slate-500" /></a>)}</div> : <div className="p-6 text-xs text-slate-500">No files have been shared yet.</div>}</section>}
      </div>

      <aside className={`${workspacePanel} self-start overflow-hidden xl:sticky xl:top-20`}>
        <PanelTitle>Activity</PanelTitle>
        <div className="max-h-[590px] overflow-y-auto p-4">{comments.length ? <div className="space-y-5">{comments.map((item) => <div key={item.id} className="flex gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-blue-400/15 text-[9px] font-bold text-blue-300">{item.authorName.slice(0, 2).toUpperCase()}</span><div className="min-w-0"><div className="text-[11px]"><strong className="text-slate-200">{item.authorName}</strong><span className="ml-2 text-slate-600">{date(item.createdAt)}</span></div><p className="mt-1 text-xs leading-5 text-slate-400">{item.body}</p><button onClick={() => setSelectedId(item.song.id)} className="mt-1 text-[10px] text-blue-300">{item.song.title}{item.timestampSeconds !== null ? ` · ${time(item.timestampSeconds)}` : ""}</button></div></div>)}</div> : <div className="py-10 text-center"><MessageSquare className="mx-auto mb-3 h-5 w-5 text-slate-600" /><p className="text-xs text-slate-500">Comments and feedback will appear here.</p></div>}</div>
        {selected && !readOnly && <form action={commentAction} className="flex gap-2 border-t border-white/10 p-3"><input type="hidden" name="songId" value={selected.id} /><input type="hidden" name="authorName" value={clientName} /><input name="body" required maxLength={2000} placeholder={`Message JMT Music…`} className="min-w-0 flex-1 rounded-[5px] border border-white/15 bg-[#0b111a] px-3 text-xs text-white" /><button aria-label="Send message" className="grid h-9 w-9 place-items-center rounded-[5px] border border-white/15 text-slate-300"><Send className="h-3.5 w-3.5" /></button></form>}
      </aside>
    </div>
  </div>;
}
